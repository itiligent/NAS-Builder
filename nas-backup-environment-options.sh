#######################################################################################
# Build a flexible backuip client layer above Linux NAS storage                       #
...
# server. Wine is a Windows OS environment emulator to run Windows apps on Linux.
# If you need to run another OS for windows client backup utilities and these applications  
# exect direct access to your NAS host's disk filesystem, setup Wine or a VM with virtio-fs to 
# pass through NAS disk storage as a local Windows disk. This is very useful for defeating 
# cloud stoarge and backup software that enforces hard limitations on accessing networked or 
# removable shared storage  (i.e. Onedrive, Backblaze etc)


# VM Solution:
# Ensure VMware ESXi is configured :
# 1. Virutalisation CPU extensions are enabled for the VM
# 2. Esxi Vswitch security must allow Promiscous mode, MAC Address Ahanges & Forged Transmits for bridging to work
